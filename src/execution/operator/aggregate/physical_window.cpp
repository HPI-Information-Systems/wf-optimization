...
static unique_ptr<WindowExecutor> WindowExecutorFactory(BoundWindowExpression &wexpr, ClientContext &client,
                                                        WindowSharedExpressions &shared, WindowAggregationMode mode, bool use_filter, bool early_out) {
...
WindowGlobalSinkState::WindowGlobalSinkState(const PhysicalWindow &op, ClientContext &client)
	...
	const auto use_filter = WindowOperatorConfig::get().do_filter ;
	const auto early_out = WindowOperatorConfig::get().do_early_out;
	...
	for (idx_t expr_idx = 0; expr_idx < op.select_list.size(); ++expr_idx) {
		...
		auto wexec = WindowExecutorFactory(wexpr, client, shared, mode, use_filter, early_out);
