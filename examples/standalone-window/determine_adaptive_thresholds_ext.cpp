...
int main(int argc, char* argv[]) {
	...
	const auto result_filename = "adaptivity_thresholds_ext_" + (skewed ? std::string{"skewed_"} : std::string{""}) + (core_count == 1 ? "st" : "mt") + ".csv";
	...
	for (const auto& [run_row_count, partition_count, filename] : file_names) {
		...
		for (const auto level : {WindowOperatorConfig::OptimizationLevel::ShrinkPartitionsAdaptive, WindowOperatorConfig::OptimizationLevel::ShrinkPartitionsAdaptiveContext}) {
			...
			auto thresholds = std::vector<double>{};
			if (level == WindowOperatorConfig::OptimizationLevel::ShrinkPartitionsAdaptive) {
				thresholds.push_back(1.8);
				thresholds.push_back(2.2);
				thresholds.push_back(2.4);
				thresholds.push_back(2.6);
			} else if (level == WindowOperatorConfig::OptimizationLevel::ShrinkPartitionsAdaptiveContext) {
